# Prompt 1 – Add SSH client helper + dependency

Use this prompt as-is with your AI agent.

---

You are working in a Python project with this structure (excerpt):

- `to_be_integrated/app.py`
- `to_be_integrated/core/...`
- `to_be_integrated/data/base.py`
- `to_be_integrated/util/ringbuf.py`
- (Tk GUI at repo root: `main.py` already has an `SSHClientManager` using `paramiko`)

I want to introduce an SSH helper that can be reused from the Qt side (and later by other modules).

## Task 1 – Add `paramiko` dependency for the Qt app

Edit `to_be_integrated/requirements.txt` and **append** `paramiko` at the bottom (don’t change anything else):
...
## Task 2 – Create `to_be_integrated/util/ssh_client.py`

Create a new module `to_be_integrated/util/ssh_client.py` that encapsulates SSH + SFTP operations.

Implement it as a small, UI‑agnostic helper (no Tk / Qt imports) with the same behaviour as the `SSHClientManager` that already exists in `main.py`. Use the code below as the implementation (you can tweak type hints or docstrings, but keep the API compatible):

```python
# to_be_integrated/util/ssh_client.py
...
After making these changes, ensure imports such as `from util.ssh_client import SSHClientManager` will work from inside the `to_be_integrated` package.
